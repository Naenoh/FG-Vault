  ...
  apollo: {
    ...
    filteredPosts: {
      query: gql`query getFilteredPosts($page: Int, $title: String, $gameId: Int, $charId: Int, $catIds: [Int]){
          filteredPosts(page: $page, title:$title,gameId:$gameId,charId:$charId, catIds:$catIds){posts{id title description game{id name} char{id name} categories{id name} links{url} timeCreated} lastPage}
       }`,
      variables () {
